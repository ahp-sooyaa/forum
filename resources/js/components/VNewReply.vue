    ...
    <div v-if="$signIn">
      ...
      >
        ...
        <div class="mb-3">
          <textarea
            id="body" name="body"
            rows="6" class="bg-light-secondary text-black dark:bg-dark-secondary dark:text-white border-0 focus:ring-0 block w-full sm:text-sm px-0"
        ...
        <button @click="addReply" class="btn-accent text-sm mr-2">Post</button>
        <button @click="isOpen = false" class="text-black dark:text-white text-sm">
          Cancel
        </button>
      </div>
    </div>
    <div v-else class="text-center mt-8">
      Please <a href="/login" class="text-black dark:text-white underline">Login</a> to participate in forum discussion!
    </div>
  </div>
</template>
