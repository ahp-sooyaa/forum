    ...
    <ais-panel class="card mr-5 rounded-2xl p-4 h-full w-1/5">
      ...
      <ais-state-results>
        <p
          ...
          :class="!hits.length ? 'hidden' : ''"
        />
        <h5 class="mt-4 mb-2 font-bold tracking-wider text-xl">
          Channels
        </h5>
      </ais-state-results>
      ...
      <ais-hits>
        ...
        >
          <li
            v-for="item in items"
            :key="item.objectID"
            class="ais-Hits-item flex items-start"
          >
            <a
              :href="'/profiles/'+item.creator.name"
              class="md:mr-5 flex items-center"
            >
              <img
                class="flex-shrink-0 order-first rounded-xl w-16 h-16"
                :src="'https://gravatar.com/avatar/'+md5(item.creator.email)+'?s=128'"
                :alt="item.creator.name+'s avatar'"
              >
            ...
            <div class="w-full">
              <div class="flex justify-between">
                <div class="flex">
                  <div
                    v-for="date in moment(item.created_at).split(' ')"
                    :key="date.id"
                  >
                    <div class="bg-white text-black dark:bg-black dark:text-white rounded-lg p-2 mr-2 mb-2 text-xs font-semibold shadow-md">
                      {{ date }}
                    </div>
                  </div>
                </div>
              ...
              <h1>
                ...
                >
                  <ais-highlight
                    class="hover:underline mb-3 block"
                    attribute="title"
                    :hit="item"
                  />
                  <ais-highlight
                    class="text-sm no-underline block text-black text-opacity-50 dark:text-white dark:text-opacity-50"
                    attribute="body"
                    :hit="item"
                  />
                </a>
