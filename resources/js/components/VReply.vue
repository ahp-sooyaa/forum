<template>
  ...
  >
    <img
      ...
      :src="'https://gravatar.com/avatar/'+data.owner.email+'?s=128'"
      :alt="data.owner.name"
    >
    <div class="w-full">
      <div class="mb-3">
        <div class="flex items-center">
          ...
          <div
            v-if="isOp"
            class="inline-block h-6 bg-white text-black dark:bg-black dark:text-white rounded-xl px-2 text-sm"
          >
            <span class="align-middle">op</span>
          </div>
        </div>
        <div class="text-black text-opacity-50 dark:text-white dark:text-opacity-50 text-xs">
          Posted {{ date }}
        </div>
      </div>
      ...
      <div v-show="isEdit">
        ...
        <div class="flex">
          ...
          <button
            @click="update"
            class="text-xs font-semibold bg-light-primary dark:bg-dark-primary border-gray-400 text-gray-400 border hover:border-gray-500 rounded-xl inline-block px-2 md:px-3 ml-2"
          >
            Update
          </button>
          <button
            @click="cancel"
            class="text-xs font-semibold text-red-500 rounded-xl inline-block px-2 md:px-3 ml-2"
          >
            Cancel
          </button>
        </div>
      ...
      <div v-show="!isEdit"> 
        ...
        <div class="flex">
          ...
          <div v-if="authorize('owns', reply) && hover">
            ...
            <button
              @click="isEdit = true"
              class="border dark:border-gray-500 dark:hover:border-gray-100 dark:text-gray-300 font-semibold h-full hover:border-gray-500 inline-block md:px-3 ml-2 px-2 rounded-xl text-gray-700 text-xs"
            >
              Edit
            </button>
            <button
              @click="destroy"
              class="h-full text-xs font-semibold text-red-400 hover:text-red-500 rounded-xl inline-block px-2 md:px-3 ml-2"
            >
              Delete
            </button>
          </div>
          <button
            v-show="!isBest && authorize('owns', reply.thread) && hover" 
            @click="markBestReply" 
