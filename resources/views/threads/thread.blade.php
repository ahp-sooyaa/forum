<div
    class="px-7 pt-7 mb-3 md:hover:bg-gray-50 md:dark:hover:bg-gray-700 card hover:bg-gray-50 dark:hover:bg-gray-700 border-0 rounded-3xl">
    <div class="md:flex justify-between items-start">
        <div class="flex flex-shrink-0 justify-between items-center mb-5">
            <a href="{{ route('profile', $thread->creator->name) }}" class="md:mr-5 flex items-center">
                <img class="flex-shrink-0 order-first rounded-xl" width="64px" height="64px"
                    src="https://gravatar.com/avatar/{{ md5($thread->creator->email) }}?s=128"
            ...
            <div class="block md:hidden">
                <div class="flex text-xs md:text-sm items-center">
                    <div class="flex bg-gray-200 text-gray-500 rounded-full px-3 py-1 items-center">
                        <svg class="w-3 h-4 mb-1 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 10"
                            fill="currentColor">
                            <path
        ...
        <div class="flex-1 flex-nowrap">
            <div class="flex justify-between mb-3">
                <div class="flex-1 mr-5"> {{-- flex-1 require otherwise it will cause overflow to other flex member --}}
                    <a class="hover:underline text-lg font-semibold" href="/threads/{{$thread->channel->slug}}/{{$thread->slug}}">
                ...
                <div class="hidden md:block">
                    <div class="flex items-center">
                        <div
                            class="flex text-black text-sm text-opacity-60 dark:text-white dark:text-opacity-50 rounded-full px-2">
    ...
    <div class="-mx-7 border-t flex justify-end mt-4 px-7 py-3 rounded-b-3xl text-right text-xs">
        @can('delete', $thread)
            <form action="/threads/{{$thread->channel->slug}}/{{$thread->slug}}" method="POST">
                @csrf
                @method('DELETE')

                <button aria-label="Delete Button" class="inline-block px-3 py-1 rounded-full">
                    <svg class="w-4 text-red-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 -1 24 24"
                        stroke="currentColor">
        ...
        @endcan
        <a class="border border-gray-300 cursor-pointer font-semibold hover:bg-accent hover:border-accent hover:text-white ml-3 px-3 py-1 rounded-full text-gray-600 dark:text-gray-300"
